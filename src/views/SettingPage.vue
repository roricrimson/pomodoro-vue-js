...
const hasChanges = computed(() => {
  const currentWork = Math.round(
    pomodoro.timerDurations.value.work / (60 * 1000)
  );
  const currentShortBreak = Math.round(
    pomodoro.timerDurations.value.shortBreak / (60 * 1000)
  );
  const currentLongBreak = Math.round(
    pomodoro.timerDurations.value.longBreak / (60 * 1000)
  );
...
function loadCurrentSettings() {
  workDurationMinutes.value = Math.round(
    pomodoro.timerDurations.value.work / (60 * 1000)
  );
  shortBreakMinutes.value = Math.round(
    pomodoro.timerDurations.value.shortBreak / (60 * 1000)
  );
  longBreakMinutes.value = Math.round(
    pomodoro.timerDurations.value.longBreak / (60 * 1000)
  );
}
...
watch(
  ...
  (newDurations) => {
    ...
    if (
      workDurationMinutes.value !== storedWork ||
      ...
      <ion-toolbar class="settings-toolbar">
        ...
        <ion-buttons slot="end">
          <ion-button
            ...
            :aria-label="
              hasChanges ? 'Save settings changes' : 'No changes to save'
            "
          >
            <ion-icon :icon="save" />
          </ion-button>
    ...
    <ion-content :fullscreen="true" class="settings-content">
      <div class="settings-container">
        <ion-list class="settings-list">
          ...
          <ion-item class="duration-item">
            ...
            <div class="input-container flex items-center">
              <ion-input
                ...
                :class="{
                  'input-error':
                    workDurationMinutes < 1 || workDurationMinutes > 120,
                }"
              ...
              <ion-input
                ...
                :class="{
                  'input-error':
                    ...
                    shortBreakMinutes >= workDurationMinutes,
                }"
          ...
          <ion-item class="duration-item" lines="none">
            <ion-label position="stacked">
              <h2>Long Break</h2>
            ...
            <ion-label color="danger">
              <ion-note color="danger" role="alert" aria-live="polite">
                <ul class="validation-list">
                  <li
                    v-if="workDurationMinutes < 1 || workDurationMinutes > 120"
                  >
                    Work session: 1-120 minutes
                  </li>
                  <li v-if="shortBreakMinutes < 1 || shortBreakMinutes > 60">
                    Short break: 1-60 minutes
                  </li>
                  <li v-if="longBreakMinutes < 1 || longBreakMinutes > 120">
                    Long break: 1-120 minutes
                  </li>
                  <li v-if="shortBreakMinutes >= workDurationMinutes">
                    Short break must be less than work time
                  </li>
                  <li v-if="longBreakMinutes < shortBreakMinutes">
                    Long break must be at least as long as short break
                  </li>
                </ul>
              </ion-note>
            </ion-label>
...
<style scoped>
.settings-container {
  padding-right: calc(var(--ion-safe-area-right) + 1rem);
  padding-left: calc(var(--ion-safe-area-left) + 1rem);
}
